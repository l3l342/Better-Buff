...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.tabview.tab("        All        ").grid_columnconfigure(0, weight=1)
        ...
        with mp.Pool() as pool:
            liste = ["All" ,"Pistols", "Mid-Tier", "Rifles", "Knives", "Gloves", "Sticker"]
            for elem in liste:
                self.home_frame = customtkinter. CTkFrame(self.tabview.tab(f"        {elem}        "), width=1000, height=700)
                self.home_frame.grid(row=0, column=1, padx=(12, 10), pady=(10, 10), sticky="nsew")
                self.grid_rowconfigure(2, weight=1)
                self.grid_columnconfigure(3, weight=1)
                results = pool.imap (Frame(database = elem, home_frame=self.home_frame),liste)
